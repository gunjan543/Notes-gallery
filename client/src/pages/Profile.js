...
export default function Profile(){
    ...
    {
    Axios
    .post(`${process.env.REACT_APP_BACKEND_URL}/api/getProfileNotes`, {id:_id})
    .then( res=>{ 
    ...
    function deleteNote(id){
        ...
        Axios
        .get(`${process.env.REACT_APP_BACKEND_URL}/api/deleteNote/`+id)
        .then((response)=>{
        ...
        <section className="profile-table">
            <h1>Hello, <span style={{textTransform:"capitalize"}}>{name}</span></h1>
                ...
                <tbody>
                    ...
                    <td>{note.unit}</td>
                    <td>{note.semester}</td>
                    <td> <a href={`${process.env.REACT_APP_BACKEND_URL}/api/file/`+note.filename}>Preview</a></td>
